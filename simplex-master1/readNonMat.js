...
function equalConstrMsg(j, countOfEq) {
  tempStr += "Restricción de división ";
  tempStr += j + 1 - countOfEq;
  tempStr += " en &leq; y &geq; restricciones. &geq; la restricción debe ser ";
  tempStr +=
    "multiplicado por -1 para convertirlo en &leq; restricción por lo que ";
  tempStr += "luego puede convertirse a forma canónica y agregarse a la ";
  tempStr += "tabla inicial.<br/><br/> ";
}
...
function geqConstrMsg(j, countOfEq) {
  tempStr += "Se multiplica la restricción ";
  tempStr += j + 1 - countOfEq;
  tempStr += " por -1 para reemplazar  &geq; con &leq;, que luego puede ser";
  tempStr += "convertida a forma canónica y añadida a la tabla inicial";
  tempStr += ".<br/><br/>";
}
...
function printEqn(
  ...
  texStr = texStr.replace(NLReg, (match, number) => number + "\\\\\n&");
  ...
  if (dualCheck) {
    tempStr += "<br/>Resolviendo.<br/><br/>";
  }

  tempStr += "El problema es:<br/>";
...
function probTypeCheck(maxReg, minReg, objVarName, type) {
  if (type.match(maxReg)) {
    ...
    type = -1;
    tempStr += "Se mulgiplica ";
    tempStr += objVarName;
    tempStr += " por -1 para obtener la maximización del problema.<br/><br/>";
  } else {
